...
'''This is a module of methods to use for manipulating and analyzing structures. Some of the methods are similar to those used in aselite, but done in a way I understand'''
...
def alignment_params(vecA,vecB):
	...
	Returns angle in radians and a numpy vector of the rotation axis"""
...
#		print "pre-rotation coord: "+str(newcoords[i])
		newcoords[i] = np.dot( rot_mat, coord )
#		print "     rotated coord: "+str(newcoords[i])
	return newcoords
...
def shift_coords(coord_list, vector):
	...
	newcoords = copy.deepcopy(coord_list)
#	print "shifting by "+str(vector)
	for i, coord in enumerate(coord_list):
#		print "pre-shifted coord: "+str(newcoords[i])
		newcoords[i] = coord + vector
#		print "    shifted coord: "+str(newcoords[i])
	return newcoords
...
def parse_keyword(line, keyword):
	# Parse keywords from a line
	chunks = line.replace("="," ").split()
	for j, chunk in eunmerate(chunks):
...
def read_poscar(system,filepath):
	...
	# Assign atomic symbol to each atom
	syms = []
	...
	for atom in system.atomlist:
		if atom.symbol in population:
			population[atom.symbol] += 1
			atom.id = atom.symbol+str(population[atom.symbol])
		else:
			population[atom.symbol] = 1
			atom.id = atom.symbol+str(population[atom.symbol])
...
#		system.atomlist[i].id = "{0}{1}".format(system.atomtypes[atomtype],str(1+i-sum1))
	...
	# Make dict for referencing atom objects by atom_id
	system.atomdict = system.get_atomdict()
...
class Structure:
	...
	def get_atomdict(self):
		# Return a dictionary to access atom objects by id
		atomdict = {}
...
#	def add_spin_to_incar(self,filename="INCAR"):
#		incar = open(filename,'r+')
#		lines = incar.readlines()
#
#		# Check for ISPIN and MAGMOM tags
#		for line in lines:
#			if "ISPIN" in line:
#				
#		incar.seek(0,2)
...
#			read_poscar(self,structurefile)
#		else:
#			print "Unable to read vasp structure file"


#	def find_molecules(self):
#		"""Method for finding a list of molecules based on ionic radii"""
#		pass
#
#	def find_dihydrogen_bonds(self,distance_cutoff):
#		"""Find a list of H-H contacts shorter than distance_cutoff"""
#		pass
